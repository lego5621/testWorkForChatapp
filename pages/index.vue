<template>
  <div>
    <Chat
      ...
      v-bind:messageArr="this.messageArr"
      v-on:closeMessage="showMessage"
    />
    ...
    <div id="main-div">
      <div
        id="main-button"
        class="wave"
        v-bind:class="{ open: isShowIcons }"
        v-on:click="addClass()"
      >
        <i
          class="fas fa-comments"
          v-bind:class="{ 'fa-times': isShowIcons }"
        ></i>
      </div>
      <a href="#" class="telegram-color" v-on:click="showMessage()">
        <i class="fab fa-telegram-plane"> </i
      ></a>
      <a href="#" class="whatsapp-color" v-on:click="showMessage()">
...
export default {
  ...
  created: function () {
    fetch(this.url, {
      ...
      .then((data) => {
        return new Pusher("ChatsAppApiProdKey", {
      ...
      .then((pusher) => {
        pusher
          .subscribe(`private-v1.licenses.${this.licenses}.messengers.telegram`)
          .bind("message", (data) => {
        ...
        pusher
          .subscribe(
            `private-v1.licenses.${this.licenses}.messengers.grayWhatsApp`
          )
          .bind("message", (data) => {
            this.messageArr.push(data);
          });
...
#main-button {
  ...
  width: 50px;
  font-size: 20px;
  color: #0064f3 !important;
  cursor: pointer;
  background-color: rgb(198, 203, 231);
